...
from sr_utils.fp16_util import convert_module_to_f16, convert_module_to_f32
from .nn import (
    checkpoint,
    conv_nd,
    linear,
...
# ytxie: This class is a base class.
...
class ResBlock(TimestepBlock):
    ...
    # ytxie: In ResBlock, time is one of the input as the form of embedding.
    def forward(self, x, emb):
        """
...
# ytxie: This QKV attention module is different from the next module.
...
class SuperResModel(UNetModel):
    ...
    # ytxie: The network inputs contain low_res
...
# ytxie: This class is used for classifier.
class EncoderUNetModel(nn.Module):
    ...
    # ytxie: The network inputs are x and t
    def forward(self, x, timesteps):
        ...
        if self.pool.startswith("spatial"):
            results.append(h.type(x.dtype).mean(dim=(2, 3)))
            # h = th.cat(results, axis=-1)  # source
            h = th.cat(results, dim=-1)  # new
            return self.out(h)
        else:
            h = h.type(x.dtype)
            return self.out(h)
